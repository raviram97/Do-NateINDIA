...
import com.example.rv.do_nateindia.model.User;
...
public class RegisterActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void postDataToSQLite(){
        if(inputValidation.isInputFilled(textInputEditTextName, textInputLayoutName, getString(R.string.error_name)))
            return;
        ...
        if(inputValidation.isInputFilled(textInputEditTextPassword, textInputLayoutPassword, getString(R.string.error_password)))
            return;
        if(!inputValidation.isInputEditTextMatches(textInputEditTextPassword, textInputEditTextConfirmPassword, textInputLayoutConfirmPassword, getString(R.string.error_passwordMismatch)))
            return;
        ...
        if(!databaseHelper.checkUser(textInputEditTextEmail.getText().toString().trim())){
            ...
            Snackbar.make(nestedScrollView, getString(R.string.registered), Snackbar.LENGTH_LONG).show();
            //Toast.makeText(getApplicationContext(), getString(R.string.registered), Toast.LENGTH_LONG).show();

            emptyInputEditText();
        }
        else{
            Snackbar.make(nestedScrollView, getString(R.string.error_registered), Snackbar.LENGTH_LONG).show();
            //Toast.makeText(getApplicationContext(), getString(R.string.error_registered), Toast.LENGTH_LONG).show();
        }
    }
